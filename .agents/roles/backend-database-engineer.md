...
For specialized tasks, consult the matching playbook under `.agents/roles/playbooks/backend/`:
- **API Design & Contracts**: `.agents/roles/playbooks/backend/api-design-and-contracts.md` (endpoint design, compatibility, API testing)
- **Database Schema & Migrations**: `.agents/roles/playbooks/backend/database-schema-and-migrations.md` (tables, constraints, safe migrations, performance)
- **Integrations & Webhooks**: `.agents/roles/playbooks/backend/integrations-and-webhooks.md` (third-party clients, inbound webhooks, auth)
- **Batch Jobs & Queues**: `.agents/roles/playbooks/backend/batch-jobs-and-queues.md` (queue workers, batch jobs, CLI utilities)
- **Telemetry & Ingestion**: `.agents/roles/playbooks/backend/telemetry-and-device-ingestion.md` (telemetry, time series, device ingestion)
